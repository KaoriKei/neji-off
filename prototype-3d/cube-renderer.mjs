...
export class CubeScene extends MetalScene {
  ...
  makeFace(face){
    ...
    p.screws.forEach((s,i)=>{
      const id=`${p.id}-${i}`,[u,v]=face.screws[i],sg=new THREE.Group();sg.position.set(u,v,.15);sg.scale.setScalar(1.30);
      ...
      sg.rotation.z=i*.32;group.add(sg);this.screws.set(id,{group:sg,base:sg.position.clone(),plateId:p.id,color:s.color,pulled:false,covered:false,button:null,spinning:false});
    });
  }
  makeFlap(def){
  ...
  bindInputs(){
    ...
    const cancel=()=>{down=null;this.stage.classList.remove('dragging');};this.stage.addEventListener('pointercancel',cancel);this.stage.addEventListener('lostpointercapture',cancel);
  }
  ...
  sync(board){
    ...
    for(const[id,s]of this.screws){s.pulled=board.getScrew(id).pulled;s.covered=board.isCovered(id);s.group.position.copy(s.base);s.group.scale.setScalar(1.30);s.spinning=false;s.group.visible=!s.pulled&&!s.covered;}
  }
  async drop(id){
